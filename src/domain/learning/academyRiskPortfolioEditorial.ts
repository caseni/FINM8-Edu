...
const COPY: Readonly<Record<string, LessonCopy>> = {
  ...
  'lesson.portfolio.construction.001': {
    ...
    takeaway: p('Amaç → sınırlar → dağılım → risk kontrolü → düzenli gözden geçirme.', 'A simple portfolio sequence is: goal → limits → allocation → risk check → review over time.'),
