...
const uncertainty = createFoundationLesson({
  ...
  title: 'Risk, belirsizlik ve kayıp aynı şey mi?', objective: 'Risk olasılığı, belirsizlik ve gerçekleşmiş kaybı ayırt eder.', minutes: 4,
  hook: 'Henüz zarar oluşmadan da risk var mıdır?',
  ...
  misconception: 'Yaygın hata: Henüz zarar olmadığı için risk de olmadığını düşünmek.',
  takeaway: 'Kayıp gerçekleşmiş sonuçtur; risk ise karar anında taşınan olumsuz sonuç ihtimalidir.',
  prerequisiteConceptKeys: [], relatedConceptKeys: ['risk.volatility', 'risk.position_sizing'],
  taskPrompt: 'Hangisi gerçekleşmiş kayıp değil, karar anındaki riski anlatır?',
  taskChoices: [{ id: 'possible', label: 'Sonucu belirsiz yatırımın değer kaybetme ihtimali' }, { id: 'realized', label: 'Hesaba geçmiş 1.000 TL zarar' }, { id: 'guaranteed', label: 'Kesin ve değişmez getiri' }], taskCorrectIds: ['possible'],
  questions: [
    { prompt: 'Finansal risk en temel olarak neyi içerir?', choices: [{ id: 'a', label: 'Belirsizlik ve potansiyel finansal kaybı' }, { id: 'b', label: 'Yalnız gerçekleşmiş zararı' }, { id: 'c', label: 'Kesin kazancı' }], correctId: 'a', explanation: 'Risk karar anında gelecekteki olumsuz sonuç ihtimalini içerir.' },
    { prompt: 'Risk yönetiminin gerçekçi amacı nedir?', choices: [{ id: 'a', label: 'Riski ve olası etkiyi sınırlandırmak' }, { id: 'b', label: 'Tüm kayıpları garantiyle yok etmek' }, { id: 'c', label: 'Her fırsatı kullanmak' }], correctId: 'a', explanation: 'Hiçbir yöntem tüm piyasa riskini ortadan kaldırmaz.' },
    { prompt: 'Belirsizlik neyi anlatır?', choices: [{ id: 'a', label: 'Sonuçları veya olasılıkları tam bilememeyi' }, { id: 'b', label: 'Kesinleşmiş hesap bakiyesini' }, { id: 'c', label: 'Yalnız fiyat artışını' }], correctId: 'a', explanation: 'Belirsizlik bilgi sınırlarını ifade eder.' },
...
const volatility = createFoundationLesson({
  ...
  title: 'Volatilite fırsat değil önce risktir', objective: 'Fiyat hareket genişliğinin pozisyon riskini nasıl etkilediğini açıklar.', minutes: 4,
  hook: 'Fiyatın hızlı hareket etmesi yalnızca daha çok fırsat mı demektir?',
  ...
  proExplanation: 'Volatilite yön söylemez. Realized ve implied ölçümler farklıdır; rejim değişimi, gap ve likidite koşulları tarihsel ölçümlerin ötesinde kuyruk riski yaratabilir.',
  misconception: 'Yaygın hata: Yüksek volatiliteyi otomatik olarak yüksek kazanç fırsatı sayıp aşağı yönlü etkiyi ihmal etmek.',
  takeaway: 'Volatilite yön tahmini değil, hareket genişliği ve risk kapasitesi bilgisidir.',
  prerequisiteConceptKeys: ['risk.basics'], relatedConceptKeys: ['risk.position_sizing', 'market.execution.slippage'],
  visualAlt: 'Aynı sermaye için düşük ve yüksek volatilite fiyat yollarının karşılaştırması',
  taskPrompt: 'Aynı büyüklükte iki pozisyondan hangisi daha yüksek kısa vadeli dalgalanma riski taşır?',
  taskChoices: [{ id: 'high-vol', label: 'Günlük hareket aralığı belirgin biçimde daha geniş olan' }, { id: 'low-vol', label: 'Dar ve istikrarlı aralıkta hareket eden' }, { id: 'logo', label: 'Logosu daha büyük olan' }], taskCorrectIds: ['high-vol'],
  questions: [
    { prompt: 'Volatilite neyi ölçmeye çalışır?', choices: [{ id: 'a', label: 'Fiyat dalgalanmasının genişliğini' }, { id: 'b', label: 'Kesin yönü' }, { id: 'c', label: 'Şirket çalışan sayısını' }], correctId: 'a', explanation: 'Volatilite hareket büyüklüğüyle ilgilidir, yönü garanti etmez.' },
    { prompt: 'Yüksek volatilitede hangisi artabilir?', choices: [{ id: 'a', label: 'Kayıp ve gerçekleşme belirsizliği' }, { id: 'b', label: 'Kesin kazanç' }, { id: 'c', label: 'Fiyat garantisi' }], correctId: 'a', explanation: 'Geniş hareketler iki yönde de riski büyütebilir.' },
    { prompt: 'Aynı pozisyon büyüklüğü farklı volatilitede aynı riski taşır mı?', choices: [{ id: 'a', label: 'Her zaman taşımaz' }, { id: 'b', label: 'Her zaman taşır' }, { id: 'c', label: 'Volatiliteyle ilgisi yoktur' }], correctId: 'a', explanation: 'Hareket aralığı büyüdükçe aynı miktarın parasal dalgalanması büyüyebilir.' },
...
const diversification = createFoundationLesson({
  ...
  proExplanation: 'Gerçek çeşitlendirme marjinal risk katkısı, korelasyonun rejimlere göre değişimi, likidite ve ortak faktör maruziyetiyle değerlendirilir. Kriz dönemlerinde korelasyonlar yakınsayabilir.',
