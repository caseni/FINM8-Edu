...
function BreakoutScene({ role, tr, styles }: SceneProps) {
  if (role === 'hook') return <><Header styles={styles} title={tr ? 'Seviyeyi geçmek yetiyor mu?' : 'Is crossing the level enough?'} detail={tr ? 'Kapanış ve ardından gelen hareket kanıtı güçlendirir.' : 'Close and follow-through can strengthen the evidence.'} /><BreakoutChart styles={styles} accepted={false} tr={tr} /></>;
  if (role === 'concept') return <><Header styles={styles} title={tr ? 'Bölgenin dışına çıkmak yetmez' : 'Move beyond the area + acceptance'} detail={tr ? 'Fiyatın bölge dışında kalıp kalmadığını izle.' : 'Assess the important area together with behavior after the break.'} /><BreakoutChart styles={styles} accepted tr={tr} /></>;
  if (role === 'practice') return <><Header styles={styles} title={tr ? 'Sadece değdi mi, dışarıda kaldı mı?' : 'Wick or acceptance?'} detail={tr ? 'Tek temas ile kalıcı hareketi ayır.' : 'Separate the two behaviors.'} /><View style={styles.row}><Card styles={styles} label={tr ? 'TEK TEMAS' : 'ONE WICK'} value="?" tone="warning" /><Card styles={styles} label={tr ? 'KAPANIŞ + DEVAM' : 'CLOSE + FOLLOW'} value="✓" tone="success" /></View></>;
  if (role === 'misconception') return <Rule styles={styles} title={tr ? 'HER TAŞMA ≠ TEYİTLİ KIRILIM' : 'EVERY OVERSHOOT ≠ CONFIRMED BREAKOUT'} detail={tr ? 'Seviyenin önemi ve devam davranışı gerekir.' : 'Level importance and follow-through matter.'} warning />;
  if (role === 'risk') return <Rule styles={styles} title={tr ? 'KIRILIM ≠ YÖN GARANTİSİ' : 'BREAKOUT ≠ DIRECTION GUARANTEE'} detail={tr ? 'Hareketin hızını, işlem kolaylığını ve baktığın süreyi de değerlendir.' : 'Assess volatility, liquidity, and timeframe separately.'} />;
  return <Rule styles={styles} title={tr ? 'KIRILIM = ÖNEMLİ BÖLGE + BAĞLAM + DEVAM' : 'BREAKOUT = IMPORTANT AREA + CONTEXT + FOLLOW-THROUGH'} detail={tr ? 'Tek çizgi geçişinden daha fazlasıdır.' : 'It is more than crossing one line.'} />;
}

function FalseBreakoutScene({ role, tr, styles }: SceneProps) {
  if (role === 'hook') return <><Header styles={styles} title={tr ? 'Kırdı, sonra geri döndü' : 'Broke out, then returned'} detail={tr ? 'Hareket bölge dışında tutunamamış olabilir.' : 'The move may have failed to hold outside the area.'} /><FalseBreakChart styles={styles} tr={tr} /></>;
  if (role === 'concept') return <><Header styles={styles} title={tr ? 'Dışarı çıkış → reddedilme → geri dönüş' : 'Break → rejection → return'} detail={tr ? 'Tutunma yerine reddedilme görülür.' : 'Rejection appears instead of acceptance.'} /><FalseBreakChart styles={styles} tr={tr} /></>;
  if (role === 'practice') return <><Header styles={styles} title={tr ? 'Sürdü mü, geri mi döndü?' : 'Did it hold or return?'} detail={tr ? 'Kapanış ve takip hareketine bak.' : 'Check the close and follow-through.'} /><View style={styles.row}><Card styles={styles} label={tr ? 'DIŞARIDA KALDI' : 'HELD OUTSIDE'} value={tr ? 'KIRILIM' : 'BREAK'} tone="success" /><Card styles={styles} label={tr ? 'İÇERİ DÖNDÜ' : 'RETURNED'} value={tr ? 'BAŞARISIZ?' : 'FALSE?'} tone="warning" /></View></>;
  if (role === 'misconception') return <Rule styles={styles} title={tr ? 'BAŞARISIZ KIRILIM ≠ OTOMATİK TERS İŞLEM' : 'FALSE BREAKOUT ≠ AUTOMATIC REVERSE TRADE'} detail={tr ? 'Yeni yön için devam kanıtı gerekir.' : 'The opposite direction still needs follow-through evidence.'} warning />;
  if (role === 'risk') return <Rule styles={styles} title={tr ? 'ETİKETE DEĞİL DAVRANIŞA BAK' : 'FOCUS ON BEHAVIOR, NOT LABELS'} detail={tr ? 'Tutunma veya reddedilme davranışı, kullanılan etiketten daha önemlidir.' : 'Acceptance/rejection matters more than methodology labels.'} />;
  return <Rule styles={styles} title={tr ? 'BAŞARISIZ KIRILIM = DIŞARIDA TUTUNAMAYAN HAREKET' : 'FALSE BREAKOUT = UNSUSTAINED BREAK'} detail={tr ? 'Kesin dönüş anlamına gelmez.' : 'It does not prove a reversal.'} />;
}
...
function PullbackScene({ role, tr, styles }: SceneProps) {
  ...
  if (role === 'risk') return <Rule styles={styles} title={tr ? 'DERİNLİK TEK BAŞINA YETERLİ DEĞİL' : 'DEPTH ALONE IS NOT ENOUGH'} detail={tr ? 'Hareketin oynaklığını ve ana yapının bozulduğu seviyeyi de kontrol et.' : 'Also assess volatility and structural invalidation.'} />;
  return <Rule styles={styles} title={tr ? 'GERİ ÇEKİLME = ANA YAPI İÇİNDE GEÇİCİ KARŞI HAREKET' : 'PULLBACK = TEMPORARY RETRACEMENT WITHIN STRUCTURE'} detail={tr ? 'Dönüş için yapı değişimi kanıtı gerekir.' : 'A reversal requires evidence of structural change.'} />;
}
...
function RangeScene({ role, tr, styles }: SceneProps) {
  ...
  if (role === 'practice') return <><Header styles={styles} title={tr ? 'Sınırları bölge olarak oku' : 'Read boundaries as zones'} detail={tr ? 'Tek fiyat yerine tekrarlayan tepki alanlarına bak.' : 'Look for repeated reaction areas rather than one exact price.'} /><View style={styles.row}><Card styles={styles} label={tr ? 'ÜST BÖLGE' : 'UPPER'} value={tr ? 'TEPKİ' : 'REACTION'} /><Card styles={styles} label={tr ? 'ALT BÖLGE' : 'LOWER'} value={tr ? 'TEPKİ' : 'REACTION'} /></View></>;
  if (role === 'misconception') return <Rule styles={styles} title={tr ? 'YATAY ALANIN ORTASI ≠ NET YÖN' : 'RANGE MIDDLE ≠ CLEAR TREND'} detail={tr ? 'Orta alandaki küçük hareketleri büyütme.' : 'Do not overinterpret small moves in the middle.'} warning />;
  if (role === 'risk') return <Rule styles={styles} title={tr ? 'SINIR AŞIMI ≠ YENİ TREND GARANTİSİ' : 'BOUNDARY BREAK ≠ NEW TREND GUARANTEE'} detail={tr ? 'Kırılımın devam edip etmediğine yine bakmak gerekir.' : 'Breakout context still matters.'} />;
  return <Rule styles={styles} title={tr ? 'YATAY ALAN (RANGE) = SINIRLAR ARASINDA DENGE' : 'RANGE = BALANCE BETWEEN BOUNDARIES'} detail={tr ? 'Sınır ve kırılım davranışını izle.' : 'Watch boundary and breakout behavior.'} />;
}

function MomentumScene({ role, tr, styles }: SceneProps) {
  if (role === 'hook') return <><Header styles={styles} title={tr ? 'Fiyat yükseliyor ama hız azalıyor' : 'Price rises while speed fades'} detail={tr ? 'Yön aynı kalırken momentum değişebilir.' : 'Momentum can change while direction remains the same.'} /><MomentumCompare styles={styles} tr={tr} /></>;
  if (role === 'concept') return <><Header styles={styles} title={tr ? 'Hız + devam gücü' : 'Speed + persistence'} detail={tr ? 'Momentum hareketin nasıl ilerlediğini özetler.' : 'Momentum summarizes how a move progresses.'} /><MomentumCompare styles={styles} tr={tr} /></>;
  if (role === 'practice') return <><Header styles={styles} title={tr ? 'Aynı yön, farklı eğim' : 'Same direction, different slope'} detail={tr ? 'Daha düşük eğim zayıflayan momentuma işaret edebilir.' : 'A shallower slope can indicate weaker momentum.'} /><View style={styles.row}><Card styles={styles} label={tr ? 'DİK' : 'STEEP'} value={tr ? 'DAHA GÜÇLÜ' : 'STRONG'} tone="success" /><Card styles={styles} label={tr ? 'YATIK' : 'SHALLOW'} value={tr ? 'DAHA ZAYIF' : 'WEAKER'} tone="warning" /></View></>;
  ...
  if (role === 'risk') return <Rule styles={styles} title={tr ? 'UYUMSUZLUK ≠ OTOMATİK DÖNÜŞ' : 'DIVERGENCE ≠ AUTOMATIC REVERSAL'} detail={tr ? 'Yapı ve devam kanıtıyla birlikte değerlendir.' : 'Assess it with structure and follow-through.'} />;
...
function MovingAverageScene({ role, tr, styles }: SceneProps) {
  ...
  return <Rule styles={styles} title={tr ? 'HAREKETLİ ORTALAMA = GEÇMİŞ FİYATIN YUMUŞATILMIŞ ÖZETİ' : 'MOVING AVERAGE = SMOOTHED PAST PRICE'} detail={tr ? 'Bağlamı özetler, geleceği bilmez.' : 'It summarizes context; it does not know the future.'} />;
}

function BreakoutChart({ styles, accepted, tr }: { styles: ReturnType<typeof createStyles>; accepted: boolean; tr: boolean }) { return <View style={styles.chart}><View style={styles.level}/><View style={[styles.path, accepted ? styles.pathAccepted : styles.pathWick]}/><Text style={styles.levelText}>{tr ? 'DİRENÇ BÖLGESİ' : 'RESISTANCE'}</Text></View>; }
function FalseBreakChart({ styles, tr }: { styles: ReturnType<typeof createStyles>; tr: boolean }) { return <View style={styles.chart}><View style={styles.level}/><View style={styles.falsePath}/><Text style={styles.levelText}>{tr ? 'KIRILIM → GERİ DÖNÜŞ' : 'BREAK → RETURN'}</Text></View>; }
...
function MomentumCompare({ styles, tr }: { styles: ReturnType<typeof createStyles>; tr: boolean }) { return <View style={styles.row}><View style={styles.momentumCard}><View style={styles.steep}/><Text style={styles.cardLabel}>{tr ? 'DAHA GÜÇLÜ' : 'STRONGER'}</Text></View><View style={styles.momentumCard}><View style={styles.shallow}/><Text style={styles.cardLabel}>{tr ? 'DAHA ZAYIF' : 'WEAKER'}</Text></View></View>; }
