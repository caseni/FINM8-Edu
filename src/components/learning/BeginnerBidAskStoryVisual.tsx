...
function QuoteCard({
  ...
  return (
    <View
      style={[styles.quoteCard, side === 'bid' ? styles.bidCard : styles.askCard]}
...
function Gap({ tr, styles, compact = false }: { tr: boolean; styles: ReturnType<typeof createStyles>; compact?: boolean }) {
  return (
    <View style={[styles.gap, compact && styles.gapCompact]} accessibilityLabel="bid-ask-spread-gap">
      <Text style={styles.gapLabel}>{tr ? 'FARK' : 'GAP'}</Text>
      <Text style={styles.gapValue}>0,20</Text>
      <View style={styles.gapLine} />
      <Text style={styles.gapName}>SPREAD</Text>
    </View>
  );
}
    ...
    <View style={styles.scene}>
      ...
      <View style={styles.quoteRow}>
        <QuoteCard
          ...
          price="99,90"
          hint={tr ? 'Ödemeye hazır' : 'Willing to pay'}
          accessibilityLabel="bid-ask-buyer-side"
          styles={styles}
        />
        ...
        <QuoteCard
          ...
          price="100,10"
          hint={tr ? 'Kabul etmeye hazır' : 'Willing to accept'}
          accessibilityLabel="bid-ask-seller-side"
          styles={styles}
        />
      </View>
    </View>
...
function ConceptScene({ tr, styles }: { tr: boolean; styles: ReturnType<typeof createStyles> }) {
  return (
    <View style={styles.scene}>
      <Heading
        styles={styles}
        title={tr ? 'Bid alıcı tarafı, ask satıcı tarafı; aradaki mesafe spread.' : 'Bid is the buyer side, ask is the seller side; the distance is the spread.'}
      />
      <View style={styles.quoteRow}>
        <QuoteCard
          side="bid"
          label="BID"
          price="99,90"
          hint={tr ? 'En iyi alış teklifi' : 'Best buy offer'}
          accessibilityLabel="bid-ask-concept-bid"
          styles={styles}
        />
        ...
        <QuoteCard
          ...
          label="ASK"
          price="100,10"
          hint={tr ? 'En iyi satış teklifi' : 'Best sell offer'}
          accessibilityLabel="bid-ask-concept-ask"
          styles={styles}
        />
      </View>
      <View style={styles.rule}>
        <Text style={styles.ruleText}>{tr ? '100,10 − 99,90 = 0,20 spread' : '100.10 − 99.90 = 0.20 spread'}</Text>
      </View>
    </View>
  );
}
...
function ActionCard({
  ...
  return (
    <View
      style={[styles.actionCard, buy ? styles.askCard : styles.bidCard]}
      accessibilityLabel={buy ? 'bid-ask-buy-now' : 'bid-ask-sell-now'}
    >
...
function MisconceptionScene({ tr, styles }: { tr: boolean; styles: ReturnType<typeof createStyles> }) {
  return (
    <View style={styles.scene}>
      <Heading
        styles={styles}
        title={tr ? 'Son işlem fiyatı, şu anki bid ve ask ile aynı şey değildir.' : 'The last traded price is not the same thing as the current bid and ask.'}
      />
      ...
      <View style={styles.currentQuotes} accessibilityLabel="bid-ask-current-quotes">
        <View style={styles.currentSide}>
          <Text style={styles.currentLabel}>BID</Text>
          <Text style={[styles.currentPrice, styles.bidText]}>99,90</Text>
        </View>
        <Text style={styles.notEqual}>≠</Text>
        <View style={styles.currentSide}>
          <Text style={styles.currentLabel}>ASK</Text>
          <Text style={[styles.currentPrice, styles.askText]}>100,10</Text>
        </View>
      </View>
    </View>
  );
  ...
  return (
    <View style={styles.scene}>
      ...
      <View style={styles.summaryStrip}>
        <View style={[styles.summaryCell, styles.bidCard]}>
          <Text style={styles.summaryLabel}>BID</Text>
          <Text style={[styles.summaryPrice, styles.bidText]}>99,90</Text>
          <Text style={styles.summaryHint}>{tr ? 'Alıcı' : 'Buyer'}</Text>
        </View>
        <View style={styles.summaryGap}>
          <Text style={styles.summaryGapValue}>0,20</Text>
          <Text style={styles.summaryGapLabel}>SPREAD</Text>
        </View>
        <View style={[styles.summaryCell, styles.askCard]}>
          <Text style={styles.summaryLabel}>ASK</Text>
          <Text style={[styles.summaryPrice, styles.askText]}>100,10</Text>
          <Text style={styles.summaryHint}>{tr ? 'Satıcı' : 'Seller'}</Text>
        </View>
      </View>
      <View style={styles.rule}>
        <Text style={styles.ruleText}>{tr ? 'Hemen alım ask’e, hemen satım bid’e yaklaşır.' : 'Immediate buying reaches toward ask; immediate selling reaches toward bid.'}</Text>
      </View>
    </View>
  );
}
...
const createStyles = (theme: LearningTheme, wide: boolean, narrow: boolean) => StyleSheet.create({
  shell: {
    width: '100%',
    minHeight: wide ? 300 : narrow ? 242 : 260,
  ...
  quoteRow: { flexDirection: 'row', alignItems: 'stretch', gap: narrow ? 5 : 8 },
  quoteCard: { flex: 1, minWidth: 0, minHeight: wide ? 154 : narrow ? 118 : 132, alignItems: 'center', justifyContent: 'center', gap: narrow ? 5 : 7, padding: narrow ? 7 : 9, borderRadius: 14, borderWidth: 1 },
  bidCard: { borderColor: '#2E716B', backgroundColor: '#0C2B31' },
  askCard: { borderColor: '#725851', backgroundColor: '#2A2025' },
  person: { width: narrow ? 34 : 40, height: narrow ? 34 : 40, alignItems: 'center', justifyContent: 'center', borderRadius: 20 },
  personBid: { backgroundColor: '#143F42' },
  personAsk: { backgroundColor: '#422D31' },
  personHead: { width: narrow ? 9 : 11, height: narrow ? 9 : 11, borderRadius: 6, backgroundColor: '#C6D3D9' },
  personBody: { width: narrow ? 18 : 21, height: narrow ? 10 : 12, marginTop: 2, borderTopLeftRadius: 9, borderTopRightRadius: 9, backgroundColor: '#8195A1' },
  quoteLabel: { color: theme.colors.textMuted, fontSize: narrow ? 8 : 9, lineHeight: 12, fontWeight: '900', letterSpacing: 0.6 },
  quotePrice: { fontSize: wide ? 22 : narrow ? 15 : 18, lineHeight: wide ? 28 : narrow ? 20 : 23, fontWeight: '900' },
  quoteHint: { color: theme.colors.textMuted, fontSize: narrow ? 8 : 9, lineHeight: narrow ? 11 : 13, textAlign: 'center' },
  bidText: { color: '#68C4B8' },
  askText: { color: '#D29A86' },
  gap: { width: wide ? 92 : narrow ? 54 : 68, alignItems: 'center', justifyContent: 'center', gap: narrow ? 3 : 4 },
  gapCompact: { width: wide ? 84 : narrow ? 50 : 62 },
  gapLabel: { color: theme.colors.textMuted, fontSize: narrow ? 7 : 8, fontWeight: '900', letterSpacing: 0.5 },
  gapValue: { color: theme.colors.text, fontSize: wide ? 16 : narrow ? 11 : 13, fontWeight: '900' },
  gapLine: { width: '72%', height: 2, borderRadius: 2, backgroundColor: '#667E8D' },
  gapName: { color: theme.colors.primary, fontSize: narrow ? 7 : 8, fontWeight: '900', letterSpacing: 0.45 },
  rule: { paddingHorizontal: wide ? 16 : 11, paddingVertical: wide ? 11 : 9, borderRadius: 11, borderWidth: 1, borderColor: '#315A61', backgroundColor: '#0D2933' },
  ...
  actionCard: { flex: 1, minWidth: 0, minHeight: wide ? 156 : narrow ? 118 : 132, alignItems: 'center', justifyContent: 'center', gap: narrow ? 7 : 9, padding: narrow ? 8 : 10, borderRadius: 14, borderWidth: 1 },
  ...
  lastTrade: { alignItems: 'center', justifyContent: 'center', gap: 3, paddingVertical: narrow ? 9 : 11, borderRadius: 12, borderWidth: 1, borderColor: '#344C5D', backgroundColor: '#102231' },
  ...
  currentQuotes: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: narrow ? 10 : 16, paddingVertical: narrow ? 8 : 10 },
  ...
  summaryStrip: { flexDirection: 'row', alignItems: 'stretch', gap: narrow ? 6 : 8 },
  summaryCell: { flex: 1, minWidth: 0, minHeight: wide ? 126 : narrow ? 96 : 108, alignItems: 'center', justifyContent: 'center', gap: 4, borderRadius: 13, borderWidth: 1 },
  ...
  summaryGap: { width: wide ? 106 : narrow ? 64 : 78, alignItems: 'center', justifyContent: 'center', gap: 3, borderRadius: 12, borderWidth: 1, borderColor: '#455765', backgroundColor: '#142331' },
  summaryGapValue: { color: theme.colors.text, fontSize: wide ? 16 : narrow ? 11 : 13, fontWeight: '900' },
  summaryGapLabel: { color: theme.colors.primary, fontSize: narrow ? 7 : 8, fontWeight: '900', letterSpacing: 0.5 },
});
