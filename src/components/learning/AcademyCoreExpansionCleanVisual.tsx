...
export function AcademyCoreExpansionCleanVisual({ assetRef, alt, language, role, theme = defaultLearningTheme }: AcademyCoreExpansionCleanVisualProps) {
  ...
  const warn = role === 'risk' || role === 'misconception';
  return <View style={[s.shell, role === 'hook' && s.shellHook, role === 'summary' && s.shellSummary, scene.kind === 'matrix' && role !== 'summary' && s.shellMatrix]} accessibilityRole="image" accessibilityLabel={alt}>{render(scene,s,warn)}</View>;
}
