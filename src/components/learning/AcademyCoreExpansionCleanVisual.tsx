...
function sceneFor(assetRef: string, tr: boolean, role: LessonSupportingVisualRole): Scene | undefined {
  ...
  // Economy expansion
  if (assetRef.includes('issizlik-verisi-ne-anlatir')) return { kind: 'matrix', labels: [tx(tr,'İSTİHDAM','EMPLOYED'),tx(tr,'İŞSİZ','UNEMP.'),tx(tr,'KATILIM','PARTIC.'),tx(tr,'DIŞARIDA','OUTSIDE')] };
...
export function AcademyCoreExpansionCleanVisual({ assetRef, alt, language, role, theme = defaultLearningTheme }: AcademyCoreExpansionCleanVisualProps) {
  ...
  const warn = role === 'risk' || role === 'misconception';
  return <View style={[s.shell, role === 'hook' && s.shellHook, role === 'summary' && s.shellSummary]} accessibilityRole="image" accessibilityLabel={alt}>{render(scene,s,warn)}</View>;
}
...
function render(scene: Scene, s:S, warn:boolean): React.ReactNode {
  switch(scene.kind){
    ...
    case 'matrix': return <View style={s.matrix}>{scene.labels.map((label,index)=><Node key={`${label}-${index}`} s={s} label={label} good={index===0&&!warn} warn={warn&&index===2}/>)}</View>;
