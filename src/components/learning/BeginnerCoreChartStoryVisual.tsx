...
export function isBeginnerCoreChartStoryAsset(assetRef: string): boolean {
...
function MiniCandle({ up = true, tall = false, styles }: { up?: boolean; tall?: boolean; styles: Styles }) {
  return (
    <View style={[styles.miniCandleWrap, tall && styles.miniCandleWrapTall]}>
      <View style={styles.miniWick} />
      <View style={[styles.miniBody, up ? styles.candleUp : styles.candleDown, tall && styles.miniBodyTall]} />
      <View style={styles.miniWick} />
    </View>
  );
}
...
function OhlcStudy({ tr, styles }: { tr: boolean; styles: Styles }) {
  return (
    <View style={styles.ohlcCard} accessibilityLabel="chart-candle-ohlc">
      <View style={styles.ohlcLabelsLeft}>
        <Text style={styles.ohlcLabel}>{tr ? 'EN YÜKSEK' : 'HIGH'}</Text>
        <Text style={styles.ohlcLabel}>{tr ? 'KAPANIŞ' : 'CLOSE'}</Text>
      </View>
      <View style={styles.largeCandleStage}>
        <View style={styles.largeWick} />
        <View style={styles.largeBody} />
        <View style={styles.largeWick} />
      </View>
      <View style={styles.ohlcLabelsRight}>
        <Text style={styles.ohlcLabel}>{tr ? 'AÇILIŞ' : 'OPEN'}</Text>
        <Text style={styles.ohlcLabel}>{tr ? 'EN DÜŞÜK' : 'LOW'}</Text>
      </View>
    </View>
  );
}
...
function CandleStory({ tr, role, styles }: StoryProps) {
  if (role === 'hook') {
    return (
      <View style={styles.scene}>
        <Heading
          styles={styles}
          title={tr ? 'Grafikteki her mum, bir zaman parçasının fiyat hikâyesidir.' : 'Each candle tells the price story of one slice of time.'}
          body={tr ? 'Tek renk değil; nerede başladı, nereye kadar gitti ve nerede bittiğini gösterir.' : 'It shows where price started, how far it travelled, and where it finished.'}
        />
        <View style={styles.candleTimeline} accessibilityLabel="chart-candle-timeline">
          {[true, true, false, true, false, true, true].map((up, index) => <MiniCandle key={index} up={up} styles={styles} />)}
        </View>
        <View style={styles.timeStrip}><Text style={styles.micro}>{tr ? 'ÖNCE' : 'EARLIER'}</Text><View style={styles.timeLine} /><Text style={styles.micro}>{tr ? 'ŞİMDİ' : 'NOW'}</Text></View>
      </View>
    );
  }
  ...
  if (role === 'concept') {
    return (
      <View style={styles.scene}>
        <Heading styles={styles} title={tr ? 'Bir mumda dört temel fiyat vardır.' : 'A candle contains four core prices.'} />
        <OhlcStudy tr={tr} styles={styles} />
        <Text style={styles.note}>{tr ? 'Fitil aralığı gösterir; gövde açılış ile kapanış arasındaki alanı gösterir.' : 'The wick shows the range; the body shows the distance between open and close.'}</Text>
      </View>
    );
  }
    ...
    return (
      <View style={styles.scene}>
        ...
        <View style={styles.practiceOhlc}>
          <View style={styles.practicePriceColumn} accessibilityLabel="chart-candle-practice-prices">
            <PriceRow label={tr ? 'Yüksek' : 'High'} value="106" styles={styles} />
            <PriceRow label={tr ? 'Kapanış' : 'Close'} value="104" accent styles={styles} />
            <PriceRow label={tr ? 'Açılış' : 'Open'} value="100" styles={styles} />
            <PriceRow label={tr ? 'Düşük' : 'Low'} value="98" styles={styles} />
          </View>
  ...
  return (
    <View style={styles.scene}>
      <Heading styles={styles} title={tr ? 'Mumu okurken önce dört fiyatı hatırla.' : 'When reading a candle, remember the four prices first.'} />
      <View style={styles.fourPointSummary}>
        {[
          [tr ? 'AÇILIŞ' : 'OPEN', '1'],
          [tr ? 'YÜKSEK' : 'HIGH', '2'],
          [tr ? 'DÜŞÜK' : 'LOW', '3'],
          [tr ? 'KAPANIŞ' : 'CLOSE', '4'],
        ].map(([label, index]) => <View key={label} style={styles.summaryChip}><Text style={styles.summaryNumber}>{index}</Text><Text style={styles.summaryLabel}>{label}</Text></View>)}
      </View>
    </View>
  );
}
...
function PathBars({ values, down = false, styles }: { values: readonly number[]; down?: boolean; styles: Styles }) {
  return (
    <View style={styles.pathBars}>
      {values.map((height, index) => <View key={`${height}-${index}`} style={styles.pathSlot}><View style={[styles.pathBar, { height }, down && styles.pathBarDown]} /></View>)}
    </View>
  );
}
...
function TimeframeStory({ tr, role, styles }: StoryProps) {
  if (role === 'hook') {
    return (
      <View style={styles.scene}>
        <Heading styles={styles} title={tr ? 'Aynı piyasa, farklı zaman diliminde farklı görünebilir.' : 'The same market can look different on different timeframes.'} />
        <View style={styles.twoPanel}>
          <View style={styles.comparePanel} accessibilityLabel="chart-timeframe-close"><Text style={styles.eyebrow}>15 DK</Text><PathBars values={[60, 52, 45, 38, 31]} down styles={styles} /><Text style={styles.resultBad}>{tr ? 'Kısa düşüş' : 'Short decline'}</Text></View>
          <View style={styles.comparePanel} accessibilityLabel="chart-timeframe-broad"><Text style={styles.eyebrow}>{tr ? '1 GÜN' : '1 DAY'}</Text><PathBars values={[25, 34, 43, 52, 62]} styles={styles} /><Text style={styles.resultGood}>{tr ? 'Geniş yükseliş' : 'Broad rise'}</Text></View>
        </View>
      </View>
    );
  }
  ...
  if (role === 'concept') {
    return (
      <View style={styles.scene}>
        <Heading styles={styles} title={tr ? 'Zaman dilimi, bir mumun ne kadar süreyi özetlediğini belirler.' : 'The timeframe defines how much time one candle summarizes.'} />
        <View style={styles.aggregateRow} accessibilityLabel="chart-timeframe-aggregate">
          <View style={styles.smallCandles}>{[true, false, true, true].map((up, index) => <MiniCandle key={index} up={up} styles={styles} />)}</View>
          <Text style={styles.aggregateArrow}>›</Text>
          <View style={styles.aggregateResult}><MiniCandle up tall styles={styles} /><Text style={styles.aggregateText}>4 × 15 dk = 1 saat</Text></View>
        </View>
      </View>
    );
  }
  ...
  if (role === 'practice') {
    return (
      <View style={styles.scene}>
        <Heading styles={styles} title={tr ? '15 dakikada düşüş görürken günlük grafikte yükseliş yapısı sürebilir.' : 'You can see a decline on 15 minutes while the daily structure is still rising.'} />
        <View style={styles.twoPanel}>
          <View style={[styles.comparePanel, styles.warningPanel]} accessibilityLabel="chart-timeframe-practice-short"><Text style={styles.eyebrow}>15 DK</Text><PathBars values={[62, 55, 49, 41, 35]} down styles={styles} /></View>
          <View style={[styles.comparePanel, styles.accentPanel]} accessibilityLabel="chart-timeframe-practice-long"><Text style={styles.eyebrow}>{tr ? 'GÜNLÜK' : 'DAILY'}</Text><PathBars values={[25, 33, 42, 50, 61]} styles={styles} /></View>
        </View>
        <Text style={styles.note}>{tr ? 'İki gözlem aynı anda doğru olabilir; çünkü baktığın ölçek farklıdır.' : 'Both observations can be true because the scale is different.'}</Text>
      </View>
    );
  }
  ...
  if (role === 'misconception') {
    return (
      <View style={styles.scene}>
        <Heading styles={styles} title={tr ? 'Kısa vadeli kırmızı görüntü, büyük resmi otomatik silmez.' : 'A red short-term view does not automatically erase the bigger picture.'} />
        <View style={styles.scaleStack}>
          <View style={styles.broadScale} accessibilityLabel="chart-timeframe-bigger-picture"><Text style={styles.eyebrow}>{tr ? 'BÜYÜK RESİM' : 'BIGGER PICTURE'}</Text><PathBars values={[24, 33, 41, 50, 59, 68]} styles={styles} /></View>
          <View style={styles.zoomWindow} accessibilityLabel="chart-timeframe-zoom-window"><Text style={styles.eyebrow}>{tr ? 'YAKIN PLAN' : 'ZOOMED VIEW'}</Text><PathBars values={[58, 49, 42, 35]} down styles={styles} /></View>
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={styles.scene}>
      <Heading styles={styles} title={tr ? 'Aynı fiyat hareketine farklı ölçeklerden bakabilirsin.' : 'You can view the same price movement at different scales.'} />
      <View style={styles.scaleSummary}>
        {['15 DK', '1 SAAT', tr ? '1 GÜN' : '1 DAY'].map((label, index) => <View key={label} style={[styles.scaleChip, index === 2 && styles.scaleChipAccent]}><Text style={styles.scaleIndex}>{index + 1}</Text><Text style={styles.summaryLabel}>{label}</Text></View>)}
      </View>
    </View>
  );
}
...
function TrendStory({ tr, role, styles }: StoryProps) {
  if (role === 'hook') {
    return (
      <View style={styles.scene}>
        <Heading styles={styles} title={tr ? 'Trend, tek bir mumun rengi değil; fiyatın oluşturduğu yapıdır.' : 'Trend is not the color of one candle; it is the structure price creates.'} />
        <View style={styles.oneVsStructure}>
          <View style={styles.singleMove} accessibilityLabel="chart-trend-single-candle"><Text style={styles.eyebrow}>{tr ? 'TEK MUM' : 'ONE CANDLE'}</Text><MiniCandle up tall styles={styles} /></View>
          <Text style={styles.notEqual}>≠</Text>
          <View style={styles.structurePanel} accessibilityLabel="chart-trend-structure"><Text style={styles.eyebrow}>{tr ? 'TEPE + DİP YAPISI' : 'HIGH + LOW STRUCTURE'}</Text><PathBars values={[25, 48, 35, 60, 47, 72]} styles={styles} /></View>
        </View>
      </View>
    );
  }
  ...
  if (role === 'concept') {
    return (
      <View style={styles.scene}>
        <Heading styles={styles} title={tr ? 'Üç temel yapı: yükselen, düşen ve yatay.' : 'Three basic structures: rising, falling, and sideways.'} />
        <View style={styles.trendTriplet}>
          <TrendCard label={tr ? 'YÜKSELEN' : 'RISING'} values={[22, 32, 42, 52, 64]} type="up" styles={styles} />
          <TrendCard label={tr ? 'DÜŞEN' : 'FALLING'} values={[64, 54, 44, 34, 24]} type="down" styles={styles} />
          <TrendCard label={tr ? 'YATAY' : 'SIDEWAYS'} values={[42, 48, 39, 46, 43]} type="side" styles={styles} />
        </View>
      </View>
    );
  }
  ...
  if (role === 'practice') {
    return (
      <View style={styles.scene}>
        <Heading styles={styles} title={tr ? 'Yükselen trendde tepeler ve dipler birlikte yukarı taşınır.' : 'In a rising trend, both highs and lows move upward.'} />
        <View style={styles.swingBoard} accessibilityLabel="chart-trend-practice-structure">
          <PathBars values={[24, 49, 34, 61, 46, 73]} styles={styles} />
          <View style={styles.swingLabels}><Text style={styles.swingLabel}>D1</Text><Text style={styles.swingLabel}>T1</Text><Text style={styles.swingLabel}>D2 ↑</Text><Text style={styles.swingLabel}>T2 ↑</Text></View>
        </View>
        <Text style={styles.note}>{tr ? 'Sadece son harekete değil, art arda oluşan tepe ve diplerin konumuna bak.' : 'Look at the sequence of highs and lows, not only the latest move.'}</Text>
      </View>
    );
  }
  ...
  if (role === 'misconception') {
    return (
      <View style={styles.scene}>
        <Heading styles={styles} title={tr ? 'Bir yeşil mum, düşen trendi tek başına yükselen trende çevirmez.' : 'One green candle does not turn a falling trend into a rising trend by itself.'} />
        <View style={styles.trendMisconception}>
          <View style={styles.fallingContext} accessibilityLabel="chart-trend-falling-context"><PathBars values={[67, 58, 49, 40, 31]} down styles={styles} /></View>
          <View style={styles.bounceCard} accessibilityLabel="chart-trend-single-bounce"><Text style={styles.eyebrow}>{tr ? 'TEK TEPKİ' : 'ONE BOUNCE'}</Text><MiniCandle up tall styles={styles} /></View>
        </View>
        <Text style={styles.note}>{tr ? 'Trend değişimi için yapının değiştiğine dair daha fazla kanıt gerekir.' : 'A trend change needs more evidence that the structure has changed.'}</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.scene}>
      <Heading styles={styles} title={tr ? 'Trend = ardışık tepe ve diplerin oluşturduğu yapı.' : 'Trend = the structure formed by successive highs and lows.'} />
      <View style={styles.structureSummary}>
        <View style={styles.structureSummaryItem}><Text style={styles.structureArrow}>↗</Text><Text style={styles.summaryLabel}>{tr ? 'YÜKSELEN' : 'RISING'}</Text></View>
        <View style={styles.structureSummaryItem}><Text style={styles.structureArrow}>↘</Text><Text style={styles.summaryLabel}>{tr ? 'DÜŞEN' : 'FALLING'}</Text></View>
        <View style={styles.structureSummaryItem}><Text style={styles.structureArrow}>→</Text><Text style={styles.summaryLabel}>{tr ? 'YATAY' : 'SIDEWAYS'}</Text></View>
      </View>
    </View>
  );
}
...
const createStyles = (theme: LearningTheme, wide: boolean, phone: boolean) => StyleSheet.create({
  shell: {
    ...
    alignSelf: 'center',
    minHeight: wide ? 300 : phone ? 246 : 270,
  ...
  futureCompare: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: phone ? 8 : 14 },
  finishedCard: { flex: 1, minHeight: phone ? 123 : 142, alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 13, borderWidth: 1, borderColor: '#377269', backgroundColor: '#0D2B31' },
  unknownCard: { flex: 1, minHeight: phone ? 123 : 142, alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 13, borderWidth: 1, borderStyle: 'dashed', borderColor: '#596873', backgroundColor: '#111E29' },
