...
function CandleStory({ tr, role, styles }: StoryProps) {
  ...
  return <View style={styles.scene}><Heading styles={styles} title={tr ? 'Mumu okurken önce dört fiyatı hatırla.' : 'When reading a candle, remember the four prices first.'} /><View style={styles.fourPointSummary}>{[[tr ? 'AÇILIŞ' : 'OPEN', '1'], [tr ? 'YÜKSEK' : 'HIGH', '2'], [tr ? 'DÜŞÜK' : 'LOW', '3'], [tr ? 'KAPANIŞ' : 'CLOSE', '4']].map(([label, index]) => <View key={label} style={styles.summaryChip}><Text style={styles.summaryNumber}>{index}</Text><Text style={styles.summaryLabel}>{label}</Text></View>)}</View></View>;
}
...
function TimeframeStory({ tr, role, styles }: StoryProps) {
  ...
  return <View style={styles.scene}><Heading styles={styles} title={tr ? 'Aynı fiyat hareketine farklı ölçeklerden bakabilirsin.' : 'You can view the same price movement at different scales.'} /><View style={styles.scaleSummary}>{['15 DK', '1 SAAT', tr ? '1 GÜN' : '1 DAY'].map((label, index) => <View key={label} style={[styles.scaleChip, index === 2 && styles.scaleChipAccent]}><Text style={styles.scaleIndex}>{index + 1}</Text><Text style={styles.summaryLabel}>{label}</Text></View>)}</View></View>;
}
