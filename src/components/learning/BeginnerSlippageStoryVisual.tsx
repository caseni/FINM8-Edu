...
export function BeginnerSlippageStoryVisual({ alt, language, role, theme = defaultLearningTheme }: BeginnerSlippageStoryVisualProps) {
  ...
  return (
    <View style={styles.shell} accessibilityRole="image" accessibilityLabel={alt}>
      {role === 'hook' ? <Hook tr={tr} narrow={narrow} styles={styles} />
        : role === 'practice' ? <Practice tr={tr} narrow={narrow} styles={styles} />
          : role === 'misconception' ? <Misconception tr={tr} styles={styles} />
            : role === 'summary' ? <Summary tr={tr} narrow={narrow} styles={styles} />
              : <Concept tr={tr} narrow={narrow} styles={styles} />}
    </View>
  );
}
...
function Hook({ tr, narrow, styles }: { tr: boolean; narrow: boolean; styles: Styles }) {
  ...
  return (
    <View style={styles.scene}>
      ...
      {narrow ? (
        ...
        <View style={styles.desktopFlow}>{screen}<ExecutionBridge tr={tr} styles={styles} />{fill}</View>
      )}
...
function Concept({ tr, narrow, styles }: { tr: boolean; narrow: boolean; styles: Styles }) {
    ...
    <View style={styles.scene}>
      ...
      {narrow ? (
        <>
          ...
          <View style={styles.mobileMechanism}><Depth tr={tr} styles={styles} /></View>
        </>
      ) : (
        <View style={styles.conceptRow}>{expected}<Depth tr={tr} styles={styles} />{actual}</View>
      )}
...
function Practice({ tr, narrow, styles }: { tr: boolean; narrow: boolean; styles: Styles }) {
  ...
  return (
    <View style={styles.scene}>
      ...
      {narrow ? <><PricePair styles={styles}>{expected}{actual}</PricePair>{delta}</> : <View style={styles.desktopFlow}>{expected}{delta}{actual}</View>}
...
function Summary({ tr, narrow, styles }: { tr: boolean; narrow: boolean; styles: Styles }) {
  ...
  return (
    <View style={styles.scene}>
      ...
      {narrow ? <><PricePair styles={styles}>{reference}{actual}</PricePair><SummaryProcess tr={tr} styles={styles} /></> : <View style={styles.summaryRow}>{reference}<SummaryProcess tr={tr} styles={styles} />{actual}</View>}
...
const createStyles = (theme: LearningTheme, wide: boolean, narrow: boolean) => StyleSheet.create({
  ...
  pricePair: { width: '100%', flexDirection: 'row', gap: narrow ? 8 : 10, alignItems: 'stretch' },
  priceCard: { flex: 1, minWidth: 0, minHeight: wide ? 128 : narrow ? 88 : 104, alignItems: 'center', justifyContent: 'center', gap: 5, padding: narrow ? 7 : 9, borderRadius: 13, borderWidth: 1 },
  ...
  desktopFlow: { width: '100%', flexDirection: 'row', alignItems: 'center', gap: 10 },
  executionBridge: { width: wide ? 170 : narrow ? 150 : 110, alignSelf: 'center', alignItems: 'center', gap: 5 },
  ...
  conceptRow: { width: '100%', flexDirection: 'row', alignItems: 'stretch', gap: 8 },
  mobileMechanism: { width: '100%', minHeight: 96 },
  depthCard: { flex: 1, minWidth: 0, minHeight: wide ? 146 : narrow ? 96 : 120, justifyContent: 'center', gap: 8, padding: narrow ? 8 : 10, borderRadius: 13, borderWidth: 1, borderColor: '#3C5668', backgroundColor: '#0E2130' },
  ...
  deltaCenter: { width: wide ? 150 : narrow ? 150 : 90, alignSelf: 'center', alignItems: 'center', justifyContent: 'center', gap: 2, paddingVertical: narrow ? 4 : 0 },
  deltaLarge: { color: '#80D0C4', fontSize: wide ? 18 : narrow ? 13 : 14, fontWeight: '900' },
  twoPanel: { width: '100%', flexDirection: 'row', gap: narrow ? 7 : 9 },
  snapshotPanel: { flex: 1, minWidth: 0, minHeight: wide ? 146 : narrow ? 106 : 120, alignItems: 'center', justifyContent: 'center', gap: 6, padding: narrow ? 7 : 10, borderRadius: 13, borderWidth: 1, borderColor: '#506879', backgroundColor: '#102331' },
  marketPanel: { flex: 1, minWidth: 0, minHeight: wide ? 146 : narrow ? 106 : 120, alignItems: 'center', justifyContent: 'center', gap: 7, padding: narrow ? 7 : 10, borderRadius: 13, borderWidth: 1, borderColor: '#7B6553', backgroundColor: '#211D21' },
  ...
  summaryRow: { width: '100%', flexDirection: 'row', alignItems: 'center', gap: 10 },
  summaryItem: { flex: 1, minWidth: 0, minHeight: wide ? 108 : narrow ? 80 : 90, alignItems: 'center', justifyContent: 'center', gap: 5, padding: 8, borderRadius: 12, borderWidth: 1, borderColor: '#506879', backgroundColor: '#102331' },
  summaryPrice: { color: theme.colors.text, fontSize: wide ? 18 : narrow ? 13 : 15, fontWeight: '900' },
  summaryProcess: { width: wide ? 170 : narrow ? 150 : 100, alignSelf: 'center', alignItems: 'center', gap: 5 },
