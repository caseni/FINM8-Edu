...
const TIMEFRAME_15M: readonly TimeframeCandle[] = [{ bottom: 14, height: 13, bullish: true }, { bottom: 24, height: 14, bullish: true }, { bottom: 30, height: 12, bullish: true }, { bottom: 26, height: 12, bullish: false }, { bottom: 19, height: 13, bullish: false }, { bottom: 12, height: 12, bullish: false }, { bottom: 9, height: 10, bullish: false }, { bottom: 13, height: 13, bullish: true }];
const TIMEFRAME_1H: readonly TimeframeCandle[] = [{ bottom: 11, height: 15, bullish: true }, { bottom: 24, height: 16, bullish: true }, { bottom: 17, height: 17, bullish: false }, { bottom: 14, height: 14, bullish: true }];
const TIMEFRAME_1D: readonly TimeframeCandle[] = [{ bottom: 8, height: 16, bullish: true }, { bottom: 20, height: 17, bullish: true }, { bottom: 31, height: 15, bullish: true }];
function TimeframeScene({ styles, language }: SceneProps) { const tr = language === 'tr'; return <View style={styles.sceneColumn}><SceneHeader styles={styles} title={tr ? 'Aynı piyasa, farklı ölçek' : 'Same market, different scale'} detail={tr ? 'Kısa geri çekilme · geniş yapı korunabilir' : 'Short pullback · broader structure can remain intact'} /><View style={styles.timeframeStack}><TimeframeRow styles={styles} label="15m" note={tr ? 'geri çekilme' : 'pullback'} candles={TIMEFRAME_15M} /><TimeframeRow styles={styles} label="1h" note={tr ? 'geçiş' : 'transition'} candles={TIMEFRAME_1H} /><TimeframeRow styles={styles} label="1D" note={tr ? 'geniş yapı' : 'broad structure'} candles={TIMEFRAME_1D} structureTag="HH / HL" /></View><Text style={styles.sceneLabel}>{tr ? 'Zaman dilimleri çelişebilir; biri otomatik olarak üstün değildir.' : 'Timeframes can conflict; one is not automatically superior.'}</Text></View>; }
function TimeframeRow({ styles, label, note, candles, structureTag }: { styles: ReturnType<typeof createStyles>; label: string; note: string; candles: readonly TimeframeCandle[]; structureTag?: string }) { return <View style={styles.timeframeRow}><View style={styles.timeframeCopy}><Text style={styles.timeframeLabel}>{label}</Text><Text style={styles.timeframeNote}>{note}</Text></View><View style={styles.timeframeChart}><View style={styles.timeframeGridLine} />{candles.map((candle, index) => <View key={`${label}.${index}`} style={styles.timeframeCandleSlot}><View style={[styles.timeframeWick, { height: candle.height + 8, bottom: candle.bottom - 4 }]} /><View style={[styles.timeframeCandle, candle.bullish ? styles.timeframeCandleUp : styles.timeframeCandleDown, { height: candle.height, bottom: candle.bottom }]} /></View>)}</View>{structureTag ? <Text style={styles.timeframeStructureTag}>{structureTag}</Text> : <View style={styles.timeframeTagSpacer} />}</View>; }

function TrendScene({ styles, language }: SceneProps) {
  const candles = [{ bottom: 18, height: 22, bullish: false }, { bottom: 42, height: 24, bullish: true }, { bottom: 31, height: 19, bullish: false, label: 'HL' }, { bottom: 67, height: 24, bullish: true, label: 'HH' }, { bottom: 52, height: 20, bullish: false, label: 'HL' }, { bottom: 80, height: 21, bullish: true, label: 'HH' }];
  return <View style={styles.sceneColumn}><SceneHeader styles={styles} title={language === 'tr' ? 'Yön, mumdan çok yapıdır' : 'Direction is structure, not one candle'} detail={language === 'tr' ? 'Daha yüksek dipler ve tepeler' : 'Higher lows and higher highs'} /><View style={styles.trendChart}><View style={styles.trendGridLine} />{candles.map((candle, index) => <View key={index} style={styles.trendCandleSlot}><View style={[styles.trendWick, { height: candle.height + 16, bottom: candle.bottom - 8 }]} /><View style={[styles.trendCandle, candle.bullish ? styles.trendCandleUp : styles.trendCandleDown, { height: candle.height, bottom: candle.bottom }]} />{candle.label ? <Text style={[styles.trendTag, { bottom: candle.bottom + candle.height + 3 }]}>{candle.label}</Text> : null}</View>)}</View><View style={styles.trendReadout}><Text style={styles.positive}>HH + HL</Text><Text style={styles.sceneLabel}>{language === 'tr' ? 'Yükseliş yapısı · teyit ara' : 'Uptrend structure · seek confirmation'}</Text></View></View>;
}

function ZoneScene({ styles, language }: SceneProps) { const candles = [{ bottom: 68, height: 20, bullish: false }, { bottom: 48, height: 27, bullish: false }, { bottom: 17, height: 28, bullish: true, touch: true }, { bottom: 35, height: 24, bullish: true }, { bottom: 55, height: 22, bullish: false }, { bottom: 17, height: 30, bullish: true, touch: true }, { bottom: 42, height: 26, bullish: true }, { bottom: 30, height: 21, bullish: false }, { bottom: 17, height: 27, bullish: true, touch: true }, { bottom: 45, height: 25, bullish: true }]; return <View style={styles.sceneColumn}><View style={[styles.zone, styles.resistanceZone]}><Text style={styles.zoneText}>{language === 'tr' ? 'DİRENÇ BÖLGESİ' : 'RESISTANCE ZONE'}</Text></View><View style={styles.zoneCandles}><View style={styles.zoneGridLine} />{candles.map((candle, index) => <View key={index} style={styles.zoneCandleSlot}><View style={[styles.zoneWick, { height: candle.height + 16, bottom: candle.bottom - 8 }]} /><View style={[styles.zoneCandle, candle.bullish ? styles.zoneCandleUp : styles.zoneCandleDown, { height: candle.height, bottom: candle.bottom }]} />{candle.touch ? <View style={styles.supportTouch} /> : null}</View>)}</View><View style={[styles.zone, styles.supportZone]}><Text style={styles.zoneText}>{language === 'tr' ? 'DESTEK BÖLGESİ · 3 TEMAS' : 'SUPPORT ZONE · 3 TOUCHES'}</Text></View><Text style={styles.sceneLabel}>{language === 'tr' ? 'Bölge tepkiyi garanti etmez; bağlamı kontrol et.' : 'A zone does not guarantee a reaction; check the context.'}</Text></View>; }
function StructureScene({ styles, mode, language }: { styles: ReturnType<typeof createStyles>; progress: Animated.Value; mode: 'bos' | 'choch'; language: LearningLanguage }) { const isBos = mode === 'bos'; const level = isBos ? 61 : 39; const candles = isBos ? [{ bottom: 22, height: 18, bullish: true }, { bottom: 38, height: 17, bullish: true }, { bottom: 31, height: 19, bullish: false }, { bottom: 49, height: 18, bullish: true }, { bottom: 44, height: 17, bullish: false }, { bottom: 57, height: 27, bullish: true, breaks: true }] : [{ bottom: 52, height: 18, bullish: true }, { bottom: 63, height: 17, bullish: true }, { bottom: 48, height: 18, bullish: false }, { bottom: 59, height: 18, bullish: true }, { bottom: 42, height: 18, bullish: false }, { bottom: 19, height: 29, bullish: false, breaks: true }]; return <View style={styles.sceneColumn}><SceneHeader styles={styles} title={isBos ? language === 'tr' ? 'Seviye üzerindeki kapanışı izle' : 'Watch for a close beyond the level' : language === 'tr' ? 'Korunan seviye kırılırsa yapı değişebilir' : 'Structure can change when a protected level breaks'} detail={isBos ? language === 'tr' ? 'BOS · devam ihtimali, garanti değil' : 'BOS · continuation possibility, not a guarantee' : language === 'tr' ? 'CHoCH · karakter değişimi sinyali' : 'CHoCH · character-change signal'} /><View style={styles.structureChart}><View style={[styles.structureLevelLine, { bottom: level }]} /><Text style={[styles.structureLevelText, { bottom: level + 5 }]}>{isBos ? language === 'tr' ? 'YAPI SEVİYESİ' : 'STRUCTURE LEVEL' : language === 'tr' ? 'KORUNAN DİP' : 'PROTECTED LOW'}</Text>{candles.map((candle, index) => <View key={index} style={styles.structureCandleSlot}><View style={[styles.structureWick, { height: candle.height + 16, bottom: candle.bottom - 8 }]} /><View style={[styles.structureCandle, candle.bullish ? styles.structureCandleUp : styles.structureCandleDown, { height: candle.height, bottom: candle.bottom }]} />{candle.breaks ? <Text style={[styles.structureBreakTag, isBos ? styles.structureBosTag : styles.structureChochTag, { bottom: candle.bottom + candle.height + 4 }]}>{isBos ? 'BOS' : 'CHoCH'}</Text> : null}</View>)}</View><Text style={styles.sceneLabel}>{language === 'tr' ? 'Tek kırılım tek başına teyit değildir; kapanış ve bağlamı birlikte değerlendir.' : 'One break is not confirmation by itself; assess the close and context together.'}</Text></View>; }
...
function DataQualityScene({ styles, language }: SceneProps) {
  ...
  const stronger = tr ? ['Kaynak · biliniyor', 'Zaman · güncel', 'Kapsam · tam', 'Bağlam · açık'] : ['Source · known', 'Time · current', 'Coverage · complete', 'Context · clear'];
  const weaker = tr ? ['Kaynak · belirsiz', 'Zaman · eski', 'Kapsam · eksik', 'Bağlam · kopuk'] : ['Source · unclear', 'Time · stale', 'Coverage · incomplete', 'Context · missing'];
  return <View style={styles.sceneColumn}><SceneHeader styles={styles} title={tr ? 'Veri kalitesi dört kontrolle okunur' : 'Read data quality through four checks'} detail={tr ? 'Kaynak · zaman · kapsam · bağlam' : 'Source · time · coverage · context'} /><View style={styles.diversificationRow}><ExposureCard styles={styles} title={tr ? 'DAHA GÜÇLÜ' : 'STRONGER'} items={stronger} tone="balanced" /><ExposureCard styles={styles} title={tr ? 'DAHA ZAYIF' : 'WEAKER'} items={weaker} tone="risk" /></View><Text style={styles.sceneLabel}>{tr ? 'Daha güçlü veri kesinlik değildir; karşı kanıt ve güncellik yine kontrol edilir.' : 'Stronger data is not certainty; recency and counter-evidence still need checking.'}</Text></View>;
}
