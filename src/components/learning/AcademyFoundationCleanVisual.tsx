...
export function AcademyFoundationCleanVisual({
  ...
  return (
    <View style={[s.shell, summary && s.shellSummary]} accessibilityRole="image" accessibilityLabel={alt}>
      ...
      {topic === 'etf' ? <Etf s={s} tr={tr} warn={warn} /> : null}
...
function Etf({ s, tr, warn }: { s: S; tr: boolean; warn: boolean }) {
  const assets = tr ? ['HİSSE','TAHVİL','ALTIN','NAKİT'] : ['STOCK','BOND','GOLD','CASH'];
  return <View style={s.rowCenter}><View style={[s.etfCard,warn&&s.pillWarn]}><Text style={s.etfText}>1 ETF</Text><Text style={s.micro}>{tr ? 'TEK İŞLEM' : 'ONE TRADE'}</Text></View><Arrow s={s}/><View style={s.basketGrid}>{assets.map((asset,i)=><View key={asset} style={[s.assetTile,warn&&i===3&&s.assetTileWarn]}><Text style={s.assetName}>{asset}</Text></View>)}</View></View>;
}
...
const createStyles = (theme: LearningTheme) => StyleSheet.create({
  shell:{width:'100%',minHeight:316,borderRadius:18,borderWidth:1,borderColor:'#24465C',backgroundColor:'#081725',padding:14,position:'relative',overflow:'hidden',justifyContent:'center'},
