...
function Exchange({ s, tr, warn }: { s: S; tr: boolean; warn: boolean }) {
  return <View style={s.rowCenter}><Pill s={s} text={tr ? 'ALICI' : 'BUYER'} /><Arrow s={s} /><View style={[s.hub, warn && s.hubWarn]}><View style={s.hubDot} /><View style={s.hubDot} /><View style={s.hubDot} /></View><Arrow s={s} reverse/><Pill s={s} text={tr ? 'SATICI' : 'SELLER'} /></View>;
}
...
const createStyles = (theme: LearningTheme) => StyleSheet.create({
  shell:{width:'100%',minHeight:210,borderRadius:18,borderWidth:1,borderColor:'#24465C',backgroundColor:'#081725',padding:14,position:'relative',overflow:'hidden',justifyContent:'center'},
