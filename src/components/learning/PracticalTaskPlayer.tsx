...
export function PracticalTaskPlayer({ task, language, presentationMode, theme = defaultLearningTheme, onComplete, completionLabel, eyebrow }: PracticalTaskPlayerProps) {
  ...
  const isCandleAnatomyVisual = task.assetRef?.includes('candle-ohlc') || task.assetRef?.includes('bir-mum');
  const isTimeframeContextVisual = task.assetRef?.includes('timeframes') || task.assetRef?.includes('zaman-dilimi');
  const isTrendStructureVisual = task.assetRef?.includes('trend-structure') || task.assetRef?.includes('trend-yon-mu-yapi-mi');
  const isSupportResistanceZoneVisual = task.assetRef?.includes('support-resistance-zone') || task.assetRef?.includes('destek-direnc-bolgedir');
  ...
  const hidesAnswerRevealingTeachingVisual = isMarketInstrumentsVisual || isTimeframeContextVisual || isTrendStructureVisual || isVolatilityRangeVisual || isFomoDecisionVisual || isOvertradingDecisionVisual || isConfirmationBiasVisual;

  const toggle = (choiceId: string) => {
    if (checked) return;
    setSelected((current) => current.includes(choiceId) ? current.filter((item) => item !== choiceId) : [...current, choiceId]);
  };
  ...
  return (
    <View style={styles.shell}>
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        <View style={styles.container}>
          ...
          {hidesAnswerRevealingTeachingVisual ? (
            ...
            <LearningVisual assetRef="edu://wave1/piyasa-araclari-ayni-degildir" alt={language === 'tr' ? 'Hisse, tahvil, döviz ve emtianın neyi temsil ettiğini dört basit kartla karşılaştıran eğitim görseli' : 'Training visual comparing what stocks, bonds, currencies, and commodities represent in four simple cards'} language={language} theme={theme} />
          ) : isPositionSizingVisual ? (
            <PositionSizingVisual alt={language === 'tr' ? 'Aynı risk bütçesinde dar ve geniş risk mesafesine göre pozisyon büyüklüğünün değişmesini gösteren eğitim görseli' : 'Training visual showing position size changing with tighter and wider risk distance under the same risk budget'} language={language} theme={theme} />
          ) : isRiskRewardVisual ? (
            <RiskRewardVisual alt={language === 'tr' ? 'Yüksek risk getiri oranının olasılık ve maliyet bilgisi olmadan tek başına yeterli olmadığını gösteren eğitim görseli' : 'Training visual showing that a high risk reward ratio is insufficient without probability and cost information'} language={language} theme={theme} />
          ) : isStopOrderVisual ? (
            <StopOrderVisual alt={language === 'tr' ? 'Stop seviyesinin yalnız tetikleyici olduğunu ve hızlı piyasada gerçekleşme fiyatının farklılaşabileceğini gösteren eğitim görseli' : 'Training visual showing that the stop level is only a trigger and the execution price can differ in a fast market'} language={language} theme={theme} />
          ) : isDiversificationVisual ? (
            <DiversificationVisual alt={language === 'tr' ? 'Çok sayıda benzer varlığın aynı risk kaynağına bağlı kalabildiğini ve farklı risk kaynaklarına yayılmanın yoğunlaşmayı azaltabildiğini gösteren eğitim görseli' : 'Training visual showing that many similar assets can share one risk source while spreading across different risk sources can reduce concentration'} language={language} theme={theme} />
          ) : isFomoDecisionVisual ? (
            <FomoDecisionVisual alt={language === 'tr' ? 'Yükselen fiyat ve sosyal baskı karşısında dürtüsel karar ile dur planı kontrol et ve karşı kanıt ara akışını karşılaştıran FOMO eğitim görseli' : 'FOMO training visual comparing an impulsive decision under rising price and social pressure with pause, plan check, and counter-evidence steps'} language={language} theme={theme} />
          ) : isOvertradingDecisionVisual ? (
            <OvertradingDecisionVisual alt={language === 'tr' ? 'Kayıp sonrası acele tekrar giriş ve kriter değiştirme döngüsünü dur plan kanıt ve risk kontrolleriyle karşılaştıran aşırı işlem eğitim görseli' : 'Overtrading training visual comparing rushed re-entry and changing criteria after a loss with pause, plan, evidence, and risk checks'} language={language} theme={theme} />
          ) : isConfirmationBiasVisual ? (
            <ConfirmationBiasVisual alt={language === 'tr' ? 'Yalnız destekleyen kanıtları toplamak ile görüşü çürütebilecek kanıtı da aramayı karşılaştıran onaylama yanlılığı eğitim görseli' : 'Confirmation-bias training visual comparing one-sided supporting evidence with deliberately seeking evidence that could disprove the view'} language={language} theme={theme} />
          ) : isDataQualityVisual ? (
            <LearningVisual assetRef="edu://wave1/her-veri-ayni-kalitede-degildir" alt={language === 'tr' ? 'Kaynak, zaman, kapsam ve bağlam kontrollerini daha güçlü ve daha zayıf veri örnekleriyle karşılaştıran veri kalitesi eğitim görseli' : 'Data-quality training visual comparing source, time, coverage, and context checks across stronger and weaker evidence'} language={language} theme={theme} />
          ) : isFreshnessVisual ? (
            <LearningVisual assetRef="edu://wave1/dogru-veri-ne-zaman-eskir" alt={language === 'tr' ? 'Daha önce doğru olan bir verinin yeni bir olaydan sonra yeniden kontrol edilmesi gerektiğini gösteren basit zaman çizelgesi' : 'Simple timeline showing that previously correct information may need to be checked again after a new event'} language={language} theme={theme} />
          ) : isDecisionJournalVisual ? (
            <DecisionJournalVisual alt={language === 'tr' ? 'Gözlem, yorum, fikri değiştirecek koşul ve sonucu ayrı adımlarda gösteren sade karar günlüğü eğitim görseli' : 'Simple decision-journal training visual separating observation, interpretation, what would change the view, and the later result'} language={language} theme={theme} />
          ) : task.kind === 'chart_identification' ? (
            task.assetRef ? (
              isCandleAnatomyVisual ? <CandleAnatomyVisual alt={language === 'tr' ? 'Açılış, kapanış, en yüksek ve en düşük seviyeleri gösteren eğitim mumu' : 'Training candle showing open, close, high, and low levels'} language={language} theme={theme} />
                : isTimeframeContextVisual ? <TimeframeContextVisual alt={language === 'tr' ? 'Aynı piyasada 15 dakikalık kısa geri çekilme ile günlük daha geniş yapının aynı anda görülebileceğini gösteren eğitim görseli' : 'Training visual showing that a 15-minute pullback and a broader daily structure can exist at the same time'} language={language} theme={theme} />
                : isTrendStructureVisual ? <TrendStructureVisual alt={language === 'tr' ? 'Yükseliş, düşüş ve yatay salınım dizilerini karşılaştıran eğitim grafiği' : 'Training chart comparing upward, downward, and sideways swing sequences'} language={language} theme={theme} />
                : isSupportResistanceZoneVisual ? <SupportResistanceZoneVisual alt={language === 'tr' ? 'Birden fazla tepkinin aynı destek ve direnç alanlarında toplandığını gösteren eğitim görseli' : 'Training visual showing multiple reactions clustering in support and resistance areas'} language={language} theme={theme} />
                : isBreakOfStructureVisual ? <BreakOfStructureVisual alt={language === 'tr' ? 'Fitil taşması ile anlamlı tepe üzerindeki teyit kapanışını karşılaştıran BOS eğitim görseli' : 'BOS training visual comparing a wick overshoot with a confirming close above a meaningful high'} language={language} theme={theme} />
                : isChangeOfCharacterVisual ? <ChangeOfCharacterVisual alt={language === 'tr' ? 'Yükseliş yapısında korunan anlamlı dibin karşı yönlü kapanışla kaybedilmesini gösteren CHoCH eğitim görseli' : 'CHoCH training visual showing a meaningful protected low lost by an opposing structural close'} language={language} theme={theme} />
                : isVolatilityRangeVisual ? <VolatilityRangeVisual alt={language === 'tr' ? 'Aynı başlangıç ve pozisyon büyüklüğünde dar ve geniş fiyat hareketlerini karşılaştıran volatilite eğitim görseli' : 'Volatility training visual comparing narrow and wide price movement with the same starting point and position size'} language={language} theme={theme} />
                : <LearningVisual assetRef={task.assetRef} alt={language === 'tr' ? 'Eğitim amaçlı şematik görev grafiği' : 'Schematic training task chart'} language={language} theme={theme} />
            ) : (
            ...
            {choices.map((choice) => {
              ...
              return (
                <Pressable
                  ...
                  style={[styles.choice, isSelected && styles.choiceSelected, isExpected && styles.choiceCorrect, isWrong && styles.choiceWrong]}
                >
      ...
      <View style={styles.footer}>
        <Pressable accessibilityRole="button" accessibilityLabel={actionLabel} accessibilityState={{ disabled: selected.length === 0 }} disabled={selected.length === 0} onPress={action} style={[styles.button, selected.length === 0 && styles.disabled]}>
