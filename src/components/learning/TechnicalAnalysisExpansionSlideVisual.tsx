...
function RsiScene({ role, tr, styles }: SceneProps) {
  ...
  if (role === 'concept') return <><Header styles={styles} title={tr ? '0–100 momentum özeti' : '0–100 momentum summary'} detail={tr ? 'Son yükseliş ve düşüşlerin göreli gücünü gösterir.' : 'Shows the relative strength of recent gains and losses.'} /><Oscillator styles={styles} value={63} labels /></>;
  if (role === 'practice') return <><Header styles={styles} title={tr ? 'Eşik + fiyat yapısı' : 'Threshold + price structure'} detail={tr ? 'RSI’ı tek başına değil fiyatla birlikte oku.' : 'Read RSI together with price, not alone.'} /><View style={styles.row}><Card styles={styles} label="RSI" value="74" tone="warning" /><Card styles={styles} label={tr ? 'YAPI' : 'STRUCTURE'} value="HH / HL" tone="success" /></View></>;
  ...
  return <Rule styles={styles} title={tr ? 'RSI = MOMENTUM BAĞLAMI' : 'RSI = MOMENTUM CONTEXT'} detail={tr ? 'Gelecek yön garantisi değil.' : 'Not a guarantee of future direction.'} />;
}
...
function MacdScene({ role, tr, styles }: SceneProps) {
  ...
  if (role === 'practice') return <><Header styles={styles} title={tr ? 'Trend mi, range mi?' : 'Trend or range?'} detail={tr ? 'Aynı kesişim farklı rejimde farklı kalite taşır.' : 'The same crossover carries different quality across regimes.'} /><View style={styles.row}><Card styles={styles} label={tr ? 'TREND' : 'TREND'} value={tr ? 'DAHA TEMİZ' : 'CLEANER'} tone="success" /><Card styles={styles} label="RANGE" value="WHIPSAW" tone="warning" /></View></>;
  ...
  return <Rule styles={styles} title={tr ? 'MACD = GECİKMELİ İLİŞKİ ÖZETİ' : 'MACD = LAGGING RELATIONSHIP SUMMARY'} detail={tr ? 'Yapı ve rejimle birlikte anlam kazanır.' : 'It gains meaning with structure and regime.'} />;
}
...
function MultiTimeframeScene({ role, tr, styles }: SceneProps) {
  ...
  if (role === 'practice') return <><Header styles={styles} title={tr ? 'Kısa düşüş ne olabilir?' : 'What can a short decline be?'} detail={tr ? 'Ana yükseliş yapısı korunuyorsa pullback olabilir.' : 'It can be a pullback if broader uptrend structure holds.'} /><View style={styles.row}><Card styles={styles} label="1D" value="HH / HL" tone="success" /><Card styles={styles} label="15m" value="↓" tone="warning" /></View></>;
  if (role === 'misconception') return <Rule styles={styles} title={tr ? 'HER TF AYNI YÖNÜ GÖSTERMEZ' : 'EVERY TF NEED NOT MATCH'} detail={tr ? 'Küçük yapı ile ana yapıyı birbirine karıştırma.' : 'Do not confuse internal structure with broad structure.'} warning />;
  if (role === 'risk') return <Rule styles={styles} title={tr ? 'ÇOK FAZLA TF = GÜRÜLTÜ' : 'TOO MANY TFs = NOISE'} detail={tr ? 'Her zaman dilimine net bir rol ver.' : 'Give each timeframe a clear role.'} />;
...
function PatternScene({ role, tr, styles }: SceneProps) {
  ...
  if (role === 'practice') return <><Header styles={styles} title={tr ? 'Şekilden sonra bağlama bak' : 'After the shape, inspect context'} detail={tr ? 'Önceki trend · hacim · volatilite · breakout davranışı' : 'Prior trend · volume · volatility · breakout behavior'} /><View style={styles.grid}><Tag styles={styles} text={tr ? 'TREND' : 'TREND'} /><Tag styles={styles} text={tr ? 'HACİM' : 'VOLUME'} /><Tag styles={styles} text={tr ? 'VOLATİLİTE' : 'VOLATILITY'} /><Tag styles={styles} text="BREAKOUT" /></View></>;
  if (role === 'misconception') return <Rule styles={styles} title={tr ? 'ŞEKİL ≠ KESİN HEDEF' : 'SHAPE ≠ CERTAIN TARGET'} detail={tr ? 'Formasyon hedefleri metodolojik projeksiyondur.' : 'Pattern targets are methodological projections.'} warning />;
  if (role === 'risk') return <Rule styles={styles} title={tr ? 'HINDSIGHT BIAS' : 'HINDSIGHT BIAS'} detail={tr ? 'Geçmiş grafikte kusursuz görünen şekil canlıda belirsiz olabilir.' : 'A perfect historical shape can be ambiguous in real time.'} />;
  return <Rule styles={styles} title={tr ? 'PATTERN = GEOMETRİ + BAĞLAM' : 'PATTERN = GEOMETRY + CONTEXT'} detail={tr ? 'Tahmin makinesi değildir.' : 'It is not a prediction machine.'} />;
}

function ConfluenceScene({ role, tr, styles }: SceneProps) {
  if (role === 'hook') return <><Header styles={styles} title={tr ? '5 indikatör = 5 kanıt mı?' : '5 indicators = 5 pieces of evidence?'} detail={tr ? 'Aynı fiyat verisinden türeyenler bağımsız değildir.' : 'Indicators derived from the same price data are not independent.'} /><EvidenceTree styles={styles} duplicate /></>;
  if (role === 'concept') return <><Header styles={styles} title={tr ? 'Confluence = kanıt çeşitliliği' : 'Confluence = evidence diversity'} detail={tr ? 'Farklı kaynak ve mekanizmaların ortak yorumu desteklemesi.' : 'Different sources and mechanisms support the same interpretation.'} /><EvidenceTree styles={styles} /></>;
  if (role === 'practice') return <><Header styles={styles} title={tr ? 'Hangisi daha bağımsız?' : 'Which is more independent?'} detail={tr ? 'Fiyat yapısı + hacim + volatilite' : 'Price structure + volume + volatility'} /><View style={styles.row}><Card styles={styles} label={tr ? 'ÇEŞİTLİ' : 'DIVERSE'} value="3×" tone="success" /><Card styles={styles} label={tr ? 'AYNI KAYNAK' : 'SAME SOURCE'} value="RSI·MACD·MA" tone="warning" /></View></>;
  if (role === 'misconception') return <Rule styles={styles} title={tr ? 'GÖSTERGE SAYISI ≠ KANIT SAYISI' : 'INDICATOR COUNT ≠ EVIDENCE COUNT'} detail={tr ? 'Correlated evidence’i iki kez sayma.' : 'Do not double-count correlated evidence.'} warning />;
  if (role === 'risk') return <Rule styles={styles} title={tr ? 'CONFLUENCE ≠ KESİNLİK' : 'CONFLUENCE ≠ CERTAINTY'} detail={tr ? 'Belirsizlik devam eder; risk ayrıca yönetilir.' : 'Uncertainty remains; risk is managed separately.'} />;
...
function IndicatorLimitsScene({ role, tr, styles }: SceneProps) {
  if (role === 'hook') return <><Header styles={styles} title={tr ? 'Daha çok indikatör = daha doğru?' : 'More indicators = more accurate?'} detail={tr ? 'Çoğu aynı fiyat/hacim verisini dönüştürür.' : 'Most transform the same price/volume data.'} /><EvidenceTree styles={styles} duplicate /></>;
  if (role === 'concept') return <><Header styles={styles} title={tr ? 'Dört temel sınır' : 'Four core limits'} detail={tr ? 'Gecikme · parametre · rejim · overfitting' : 'Lag · parameters · regime · overfitting'} /><View style={styles.grid}><Tag styles={styles} text={tr ? 'GECİKME' : 'LAG'} /><Tag styles={styles} text={tr ? 'PARAMETRE' : 'PARAMETER'} /><Tag styles={styles} text={tr ? 'REJİM' : 'REGIME'} /><Tag styles={styles} text="OVERFIT" /></View></>;
  if (role === 'practice') return <><Header styles={styles} title={tr ? 'Geçmişte mükemmel, sonra zayıf' : 'Perfect in-sample, weak later'} detail={tr ? 'Overfitting ihtimalini düşün.' : 'Consider overfitting.'} /><View style={styles.row}><Card styles={styles} label="BACKTEST" value="98%" tone="success" /><Card styles={styles} label={tr ? 'YENİ DÖNEM' : 'NEW PERIOD'} value="52%" tone="risk" /></View></>;
  ...
  if (role === 'risk') return <Rule styles={styles} title={tr ? 'PARAMETER MINING RİSKİ' : 'PARAMETER-MINING RISK'} detail={tr ? 'Geçmişe en iyi uyan ayar gelecekte dayanıklı olmayabilir.' : 'The setting that best fits history may not be robust later.'} />;
...
function EvidenceTree({ styles, duplicate = false }: { styles: ReturnType<typeof createStyles>; duplicate?: boolean }) {
  return <View style={styles.evidenceTree}><View style={styles.sourceNode}><Text style={styles.sourceText}>{duplicate ? 'PRICE' : 'EVIDENCE'}</Text></View><View style={styles.branchRow}>{duplicate ? <><Tag styles={styles} text="RSI" /><Tag styles={styles} text="MACD" /><Tag styles={styles} text="MA" /></> : <><Tag styles={styles} text={styles ? 'STRUCTURE' : ''} /><Tag styles={styles} text="VOLUME" /><Tag styles={styles} text="VOLATILITY" /></>}</View></View>;
}
